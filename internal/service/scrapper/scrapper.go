...
func NewScrapper(cfg *config.Config, logger *slog.Logger) Scrapper {
	return NewScrapperService(map[string]Scrapper{
		"github.com":        NewGithubScrapper(&cfg.HTTP, &cfg.CircuitBreaker, cfg.Scrapper.GithubToken, logger),
		"stackoverflow.com": NewStackoverflowScrapper(&cfg.HTTP, &cfg.CircuitBreaker, cfg.Scrapper.StackoverflowKey, logger),
	})
}
